...
def formatToolCalls(index: int, output_dict: dict, prefix: str) -> list:
    ...
    toolMetadata = {}
    try:
        with open(f"./meta/upd_tool_desc.json", 'r') as file:
...
def evaluateTestCases(prefix):
    output_dict = pd.read_csv(f'./data/{prefix}-enh-desc-axis-dataset.csv').to_dict()
    ...
    with open(f"./results/{prefix}-enh-desc-results.json", "w") as f:
