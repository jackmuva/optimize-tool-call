...
def runPrompt(test_dict, llm, sys_prompt:str="", prompt_num:int=0, tool_dict:dict = {}):
    ...
    results_dict = {}
    try:
        with open(f"./data/{llm}-enh-desc-cache.json", 'r') as file:
        ...
        if 'error' in results_dict and str(i) in results_dict['error'] and results_dict['error'][str(i)] == '':
            ...
            try:
                ...
                if i not in failures:
                    ...
                    i += 1
        with open(f"./data/{llm}-enh-desc-cache.json", "w") as outfile: 
    ...
    try: 
        ...
        populated_df.to_csv(f'./data/{llm}-enh-desc-axis-dataset.csv')
